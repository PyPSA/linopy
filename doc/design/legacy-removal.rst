...
-  ``Variable.to_linexpr``: drop the ``else`` branch (legacy
   ``reindex_like(fill_value=0).fillna(0)``); make the v1 ``reindex_like( fill_value=NaN) → .where(~absent)`` path unconditional. The
...
``grep -rn "@pytest.mark.legacy\|@pytest.mark.v1\|pytestmark = pytest.mark.legacy" test/`` finds every marker:
...
This file (``doc/design/legacy-removal.rst``)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
