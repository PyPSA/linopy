...
* Reading metadata of a CSR-backed ``LinearExpression`` (``repr()``, ``shape``, ``sizes``, ``dims``, ``coords``, ``indexes``, ``isnull()``) no longer converts it to dense; it is served from the sparse backing. The backing's auxiliary coordinates now live on its ``Grid``. (`#962 <https://github.com/PyPSA/linopy/issues/962>`__)
