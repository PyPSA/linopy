...
* You can now group a ``LinearExpression`` by a coordinate name, e.g. ``expr.groupby("period").sum()`` -- including coordinates that are not dimensions and levels of a ``MultiIndex`` -- matching ``xarray.Dataset.groupby`` (`#750 <https://github.com/PyPSA/linopy/issues/750>`__).
