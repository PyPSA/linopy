...
* You can now group a ``LinearExpression`` by a coordinate that is not a dimension, e.g. ``expr.groupby("period").sum()``, matching ``xarray.Dataset.groupby`` (`#750 <https://github.com/PyPSA/linopy/issues/750>`__).
