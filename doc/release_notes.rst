...
* ``LinearExpression.groupby`` with a **list of coordinate names** (``expr.groupby(["period", "season"]).sum()``) now takes the fast reindex path instead of silently falling back to the slow xarray implementation, returning one dimension per key as before. Pass ``observed=True`` to ``sum`` to keep the result compact over only the observed key combinations (a stacked ``MultiIndex`` ``group`` dimension) instead of materialising the dense cartesian grid (`#753 <https://github.com/PyPSA/linopy/issues/753>`__).
