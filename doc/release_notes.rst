...
* ``Model.copy`` (and the ``copy.copy``/``copy.deepcopy`` protocols) keeps a frozen ``CSRConstraint`` frozen instead of rebuilding it as a dense ``Constraint``, with its matrix, grid, sign, rhs, scaling and slack; ``deep=False`` shares its buffers and duals are copied only with ``include_solution=True``. A CSR-backed objective is now deep-copied with ``deep=True`` as well. (`#981 <https://github.com/PyPSA/linopy/issues/981>`__)
