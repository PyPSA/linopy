...
* ``model.spec.expressions`` (a ``linopy.spec.NamedExpressions`` mapping) returns a ``linopy.spec.NamedExpression`` for each declared name, with three views: ``.node`` (the lowered formula), ``.expression`` (a named expression whose body carries a variable term: the model's own ``LinearExpression`` or ``QuadraticExpression`` from ``model.expressions``; a data-only body: an array or scalar) and ``.solution`` (the expression folded over the solved model). ``model.spec.evaluate(name, sources)`` returns the same object with its parameters attached afresh.

* Every variable, constraint and expression a spec layer builds carries the layer's name in ``attrs["spec"]``, read back through a ``spec`` property (``str | None``) on ``Variable``, ``Constraint``, ``LinearExpression`` and ``QuadraticExpression``; a bound variable carries ``None``, since the layer reads it but the model owns it, and a derivative of a stamped expression (``expr * 2``, ``linopy.merge``, ``model.add_constraints(expr >= 0)``) carries none either. A named expression whose body holds a variable term is built into ``model.expressions`` under its declared name at ``add_spec`` time and stamped, so ``model.expressions[name]`` and ``model.spec.expressions[name].expression`` are one object; a data-only body or one reading a constraint's ``dual`` stays spec-only. Pass ``build_expressions=False`` to ``add_spec`` / ``from_spec`` to keep a layer's named expressions lazy instead, the escape hatch for a spec too large to build and store eagerly; the flag is not persisted, so a model read back from a file folds whatever it does not already hold. ``model.spec.unspecified.expressions`` and ``model.remove_expressions`` (which now refuses a spec-built name, like the variable and constraint removers) both track only names no layer built. ``repr(model)`` tags a variable, constraint or expression with its owning layer only on a model that also holds hand-built items beside a spec; a whole-spec model's repr carries no tags.
...
* Building now refuses what it used to build wrong or accept silently: a constraint row whose variable terms are all absent (built ``+0 = rhs`` and dropped from the LP instead of raising); a quadratic term reaching a constraint position instead of the objective; a variable bound from a *different* model; and a dimension backed by a ``pd.MultiIndex`` (flatten it, or split it into two dimensions, first).

* An ownership registry tracks every name a layer builds or binds, so ``Variables.remove`` / ``Constraints.remove`` / ``Expressions.remove`` refuse a spec-owned name directly on the container, not only through the ``Model``-level wrappers. ``model.remove_spec(name)`` unwinds one layer -- its built constraints, expressions and variables, unstamping bound variables and resetting an owned objective -- and refuses if another layer still binds one of its variables.
...
* ``linopy.spec.normalize_spec`` replaces the ad hoc source and layer-name detection: YAML text, a path, a ``Mapping``/``Spec`` and a lowered ``Program`` are told apart the same way everywhere, an empty spec is refused up front, and a layer name is checked against netcdf's naming rules before it reaches ``to_netcdf``.
...
* ``LinearExpression.groupby`` with a **list of coordinate names** (``expr.groupby(["period", "season"]).sum()``) now takes the fast reindex path instead of silently falling back to the slow xarray implementation, returning one dimension per key as before (`#753 <https://github.com/PyPSA/linopy/issues/753>`__). See ``observed`` under Features to keep the result compact instead.
