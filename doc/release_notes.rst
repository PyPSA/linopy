...
* ``Model.add_variables``: extend and finalize the 0.7.0 fix that made ``coords`` the source of truth for ``DataArray`` bounds, hardening it across the remaining bound types and dimension order. Pandas ``Series`` / ``DataFrame`` bounds missing a dimension are now broadcast to ``coords`` (previously silently dropped when both bounds were pandas, `#709 <https://github.com/PyPSA/linopy/issues/709>`__), and the variable's dimension order always follows ``coords`` regardless of bound type (previously the order of expanded array bounds depended on the type of the bound, `#706 <https://github.com/PyPSA/linopy/issues/706>`__).
