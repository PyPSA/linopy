...
* Default internal integer labels to ``int32``, cutting memory ~25% and speeding up model build 10-35%. The dtype is set per model via ``Model(label_dtype=np.int64)`` (exposed as ``Model.label_dtype``, defaulting to ``linopy.options["label_dtype"]``) and each model auto-widens itself to ``int64`` once its labels exceed the int32 maximum (~2.1 billion), emitting a ``UserWarning`` that recommends passing ``label_dtype=np.int64`` upfront for large models. The widening is per-model and travels with the model through netCDF and pickle round-trips; other models and the global option are unaffected.
