...
* ``Solution.primal`` and ``Solution.dual`` are now ``np.ndarray`` lookup arrays keyed by integer model labels (length = ``max_label + 1``, NaN for unfilled positions). Code that introspected these as ``pd.Series`` must use ``np.ndarray`` semantics. Construction via ``pd.Series`` is no longer supported.
