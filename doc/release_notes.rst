...
* Default internal integer labels to ``int32`` (configurable via ``linopy.options["label_dtype"]``, set to ``np.int64`` for the old behavior), cutting memory ~25% and speeding up model build 10-35%. Each model snapshots the option at creation (exposed as ``Model.label_dtype``) and auto-widens itself to ``int64`` once its labels exceed the int32 maximum (~2.1 billion), emitting a ``UserWarning`` that recommends setting ``linopy.options["label_dtype"] = np.int64`` upfront for large models. The widening is per-model and travels with the model through netCDF and pickle round-trips; other models and the global option are unaffected.
