...
def test_ensure_dataarray_scalar() -> None:
    ...
    da = ensure_dataarray(1, coords={"x": [10, 20]}, dims=["x"])
    assert isinstance(da, DataArray)
    assert da.dims == ("x",)
    assert list(da.coords["x"].values) == [10, 20]


def test_ensure_dataarray_numpy() -> None:
    from linopy.common import ensure_dataarray

    arr = np.array([1, 2, 3])
    da = ensure_dataarray(arr, coords={"x": [10, 20, 30]}, dims=["x"])
    assert isinstance(da, DataArray)
    assert da.dims == ("x",)
    assert list(da.coords["x"].values) == [10, 20, 30]


def test_ensure_dataarray_no_expand() -> None:
    from linopy.common import ensure_dataarray

    da_in = DataArray([1, 2], dims=["x"], coords={"x": ["a", "b"]})
    # Missing dim "y" should NOT be expanded — ensure only does type conversion
    da = ensure_dataarray(
...
def test_ensure_dataarray_allows_extra_dims() -> None:
    from linopy.common import ensure_dataarray

    da_in = DataArray([[1, 2], [3, 4]], dims=["x", "y"])
    # Should NOT raise even though "y" is not in coords
    da = ensure_dataarray(da_in, coords={"x": [0, 1]}, dims=["x"])
    assert set(da.dims) == {"x", "y"}


def test_ensure_dataarray_no_coord_validation() -> None:
    from linopy.common import ensure_dataarray

    da_in = DataArray([1, 2, 3], dims=["x"], coords={"x": [10, 20, 30]})
    # Should NOT raise even though coords don't match
    da = ensure_dataarray(da_in, coords={"x": [10, 20, 40]}, dims=["x"])
    assert list(da.coords["x"].values) == [10, 20, 30]
