...
def test_model(linear_objective, quadratic_objective):
    assert isinstance(linear_objective.model, Model)
    assert isinstance(quadratic_objective.model, Model)


def test_sense(linear_objective, quadratic_objective):
...
def test_set_sense(linear_objective, quadratic_objective):
...
def test_set_sense_via_model(linear_objective, quadratic_objective):
...
def test_set_value(linear_objective, quadratic_objective):
...
def test_set_value_error(linear_objective):
    with pytest.raises(ValueError):
        linear_objective.set_value("not a number")


def test_assign(linear_objective):
    assert isinstance(linear_objective.assign(one=1), Objective)


def test_sel(linear_objective):
...
def test_add(linear_objective, quadratic_objective):
...
def test_neg(quadratic_objective):
    obj = -quadratic_objective
    assert isinstance(obj, Objective)
    assert isinstance(obj.expression, QuadraticExpression)


def test_truediv(quadratic_objective):
    obj = quadratic_objective / 2
    assert isinstance(obj, Objective)
    assert isinstance(obj.expression, QuadraticExpression)
