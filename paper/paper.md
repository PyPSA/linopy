...
The array-based modelling approach does not only lead to more flexibility but also increases the overall performance. The following figure shows the [benchmark](https://github.com/PyPSA/linopy/tree/master/benchmark) against the AMLs JuMP and Pyomo, chosen for this comparison as they are both open source and well-established. Linopy outperforms `Pyomo` in terms of speed and memory-efficiency and outperforms JuMP in memory-efficiency.
...
while other solvers such as `PIPS-IPM++` [@rehfeldtMassivelyParallelInteriorpoint2022] and the `SCIP` solver [@bestuzhevaSCIPOptimizationSuite] are planned to be integrated in future versions. Further, upcoming features target model scaling as well as the integration of non-linear expressions.
