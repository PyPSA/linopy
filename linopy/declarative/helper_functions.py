...
"""
Functions that can be used to process data in math `mask` and `expression` strings.
...
_registry: dict[
    Literal["mask", "expression"], dict[str, type["ParsingHelperFunction"]]
] = {"mask": {}, "expression": {}}
...
class ParsingHelperFunction(ABC):
    ...
    @property
    @abstractmethod
    def ALLOWED_IN(self) -> list[Literal["mask", "expression"]]:
        """List of parseable math strings that this function can be accessed from."""

    @property
    @abstractmethod
    def NAME(self) -> str:
        """Helper function name that is used in the math expression/mask string."""

    @property
    def ignore_mask(self) -> bool:
        """If True, `mask` arrays will not be applied to the incoming data variables (valid for expression helpers)."""
        return False
...
class MaskAny(ParsingHelperFunction):
    """Apply `any` over a dimension in `mask` string."""
    ...
    def as_array(
        ...
        """
        Reduce the boolean mask array of a model input by applying `any` over some dimension(s).
...
class SelectFromLookupArrays(ParsingHelperFunction):
    ...
    def as_array(
        ...
        Raises:
            ValueError: `array` must be indexed over the dimensions given in the `lookup_arrays` dict keys.
            ValueError: All `lookup_arrays` must be indexed over all the dimensions given in the `lookup_arrays` dict keys.
        ...
        Examples:
            ...
            >>> select_from_lookup_arrays = SelectFromLookupArrays(
...
class Roll(ParsingHelperFunction):
    ...
    @property
    def ignore_mask(self) -> bool:
        """Whether or not to ignore `mask` functionality."""
        return True
...
class Mask(ParsingHelperFunction):
    """Apply `mask` array _within_ an expression string."""
        ...
        Examples:
            ...
            constraints:
                my_new_constraint:
                    ...
                    equations:
                        - expression: sum(mask(flow_cap, node_grouping), over=nodes) <= node_group_max
            ```
        """
...
class SumNextN(ParsingHelperFunction):
    ...
    def as_array(self, array: xr.DataArray, over: xr.DataArray, N: int) -> xr.DataArray:
        ...
        Note:
            ...
            - You will find that this over-constrains the model unless you limit the constraint (using the `mask` string) to only apply over `len(over) - N`.
              This is linked to the abovementioned lack of wrapping.
              E.g. `mask: timesteps<=get_val_at_index(timesteps=-24)` if N == 24.
        ...
        Examples:
            ...
            constraints:
                4hr_demand_side_response:
                    ...
                    mask: "carrier_in AND sink_use_flexible AND timesteps<=get_val_at_index(timesteps=-24)"
                    equations:
