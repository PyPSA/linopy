...
from linopy.declarative import expression_parser
...
class ConfigOptionParser(expression_parser.EvalArrayOrMath):
    ...
    def __init__(self, instring: str, loc: int, tokens: pp.ParseResults) -> None:
        ...
        Args:
            ...
            loc (int):
                Location in parsed string mask parsing error was logged.
    ...
    def as_array(self) -> xr.DataArray:  # noqa: D102, override
        ...
        if not isinstance(config_val, int | float | str | bool | np.bool_):
            raise self.error_msg(
                f"mask string | Configuration option resolves to invalid "
...
class ResultArrayParser(expression_parser.EvalArrayOrMath):
    ...
    def as_math_string(self) -> str:  # noqa: D102, override
        ...
        math_repr = self.eval_attrs.model[self.array_name].attrs.get(
...
class InputArrayParser(expression_parser.EvalArrayOrMath):
    ...
    def as_math_string(self) -> str:  # noqa: D102, override
        ...
        math_repr = self.eval_attrs.input_data[self.array_name].attrs.get(
...
class GenericStringParser(expression_parser.EvalString):
    ...
    def __init__(self, instring: str, loc: int, tokens: pp.ParseResults) -> None:
        ...
        This is required since we call "eval()" on all elements of the mask string,
...
def generate_mask_string_parser(
    dimensions: Iterable,
    inputs: Iterable,
    results: Iterable,
    ...
    """
    Creates and executes the mask parser.
