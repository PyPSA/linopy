...
class Expr(str):
    ...
    def to_string_tuples(self) -> list:
        """
        Restructure an expression that can be interpreted as a linear
        expression to a list of coefficient-variable-tuples, where all values
        are kept as strings.
    ...
    def to_constraint_args_kwargs(self) -> tuple:
        ...
        to arguments and keyword-arguments for the function
        ``Model.add_constraints`` where all values are kept as strings.
    ...
    def to_variable_kwargs(self) -> dict:
        """
        Convert expression that can be interpreted as a variable definition to
        keyword arguments for the function ``Model.add_variables`` where all
...
def separate_coeff_and_var(term: Expr) -> tuple:
    """
    Subdivide a Exprs that can be interpreted as a mathematical term into a
    coefficient-variable tuple. The expression is required to start with a
    sign.
