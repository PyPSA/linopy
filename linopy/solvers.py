...
def run_cbc(problem_fn, log_fn, solution_fn=None, warmstart_fn=None,
    ...
    data = pd.read_csv(io.BytesIO(trimmed_sol_fn), header=None, skiprows=[0],
    ...
    return dict(status=status, termination_condition=termination_condition,
                solution=solution, dual=dual, objective=objective)
...
def run_glpk(problem_fn, log_fn, solution_fn=None, warmstart_fn=None,
    ...
    if 'Marginal' in dual_:
        dual = pd.to_numeric(dual_['Marginal'], 'coerce')\
    ...
    solution = (pd.read_fwf(solution)[1:].set_index('Column name')
